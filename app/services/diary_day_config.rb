...
class DiaryDayConfig
  ...
  class << self
    ...
    def type_label(tipo)
      TIPO_LABELS[tipo.to_s]
    end

    def suggested_pause_for(day_number)
      PAUSAS_SUGERIDAS[day_number]
    end

    def reflection_day?(day_number)
      REFLEXION_DAYS.include?(day_number)
    end

    def micropause_day?(day_number)
      MICROPAUSA_DAYS.include?(day_number)
    end

    def new_type_for(day_number)
      return nil unless [3, 5, 7, 9, 11, 13].include?(day_number)

      types_for(day_number).last
    end
